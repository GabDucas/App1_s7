...
def relu_backward(X, output_grad):
    ...
    for index, val in np.ndenumerate(X):
        if X[index] > 0:
            grad_input[index] = output_grad[index]
        else:
            grad_input[index] = 0
...
def sigmoid_forward(X):
    ...
    for index, value in np.ndenumerate(X):
        sortie[index] = 1 / (1 + np.exp(-value))

    # on peut juste faire return 1/(1+np.exp(-X))
    return sortie
...
def bce_backward(x, target):
    N = len(x)
    return (1/N)*((-target/x) + (1-target)/(1-x))
...
def train(x_train, target_train, x_val=None, target_val=None, epoch_count=100, learning_rate=0.04):
    ...
    for epoch in range(epoch_count):
        ...
        dl_dy = bce_backward(y_hat, target_train)
        dl_dw = sigmoid_backward(w, dl_dy)
...
def show_decision_boundary(W1, b1, W2, b2, W3, b3):
    ...
    ax.imshow(1 - y_hat.reshape(x1.size, x2.size).T, cmap='bwr', extent=[-1, 1, -1, 1], vmin=0, vmax=1)
    fig.show()
...
if mode == 'test':
    ...
    train(x_train, target_train, x_val, target_val, epoch_count=10000, learning_rate=0.05)
